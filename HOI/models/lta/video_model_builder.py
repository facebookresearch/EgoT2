...
from utils.lta import weight_init_helper as init_helper
...
# Number of blocks for different stages given the model depth.x
...
class SlowFast(nn.Module):
    ...
    def forward(self, x, bboxes=None, middle=False):
        x = self.s1(x)
        x = self.s1_fuse(x)
        x = self.s2(x)
        x = self.s2_fuse(x)
        ...
        if hasattr(self, "head_name"):
            ...
            if hasattr(self.recognition_model, "head_name"):
                head = getattr(self.recognition_model, self.head_name)
                return head(x)
        return x
