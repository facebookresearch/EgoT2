...
class ForecastingEncoderDecoder(nn.Module):
    def __init__(self, cfg, build_decoder=True):
        super().__init__()
        self.cfg = cfg
    ...
    def aggregate_clip_features(self, x, middle=False):
        return self.clip_aggregator(x, middle)
    ...
    def forward(self, x, tgts=None, middle=False):
        features = self.encode_clips(x)  # [(bs, 4096)] list len=num_clips
        x = self.aggregate_clip_features(features, middle=middle)  # (bs, 4096)
        if middle:
            return x
