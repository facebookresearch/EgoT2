...
class RecognitionTask2Loader(VideoTask):
    checkpoint_metric = "val_top1_noun_err"

    def training_step(self, batch, batch_idx):
        inputs, labels, _, _ = batch['orig']
        inputs2 = batch['pnr']
        preds = self.model(inputs, inputs2)  # preds [(bs, 115), (bs, 478)]
    ...
    def training_epoch_end(self, outputs):
        if self.cfg.BN.USE_PRECISE_STATS and len(get_bn_modules(self.model)) > 0:
            misc.calculate_and_update_precise_bn_2loader(
                self.train_loader, self.model, self.cfg.BN.NUM_BATCHES_PRECISE
            )
            print('bn use precise stats')
            # need to fix bug for bn_stats
    ...
    def validation_step(self, batch, batch_idx):
        inputs, labels, _, _ = batch['orig']
        inputs2 = batch['pnr']
        preds = self.model(inputs, inputs2)
        top1_err_verb, top5_err_verb = metrics.distributed_topk_errors(
            preds[0], labels[:, 0], (1, 5)
        )
        top1_err_noun, top5_err_noun = metrics.distributed_topk_errors(
    ...
    def test_step(self, batch, batch_idx):
        inputs, labels, clip_id, _ = batch['orig']
        inputs2 = batch['pnr']
        preds = self.model(inputs, inputs2)
...
class LTA2Loader(VideoTask):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        preds = self.model.generate(inputs, inputs2, k=k)  # [(B, K, Z)]
        step_result = {}
    ...
    def test_epoch_end(self, outputs):
        ...
        if du.get_local_rank() == 0:
            ...
            json.dump(pred_dict, open('outputs_lta_2loader.json', 'w'))