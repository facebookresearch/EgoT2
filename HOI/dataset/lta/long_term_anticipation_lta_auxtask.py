...
class Ego4dLongTermAnticipationwithAuxTask(Ego4dLongTermAnticipation):
    def __init__(self, cfg, cfg_aux, mode):
        super(Ego4dLongTermAnticipationwithAuxTask, self).__init__(cfg, mode)
        self.mode = mode
    ...
    def __getitem__(self, index):
        ...
        for name in value[5]:
            ...
            frames = self._get_item_pnr(name)
            if frames.shape[1] < 16:
                tmp = torch.zeros((3, 16-frames.shape[1], 225, 225))
                frames = torch.concat((frames, tmp), dim=1)
            pnr_value.append(frames)
            # print(frames.shape)
        pnr_data = torch.stack(pnr_value, dim=0)  # (num_inputs, 3, 16, 225, 225)
        return {'orig': value, 'pnr': pnr_data}
...
@DATASET_REGISTRY.register()
class Ego4dLongTermAnticipationwithAuxTaskSeparateSequenceLabel(Ego4dLongTermAnticipationwithAuxTask):
    def __init__(self, cfg, cfg_aux, vocab, mode):
        super(Ego4dLongTermAnticipationwithAuxTaskSeparateSequenceLabel, self).__init__(cfg, cfg_aux, mode)
        self.vocab = vocab
    ...
    def __getitem__(self, index):
        ...
        orig_new = inputs, torch.LongTensor(target_seq_verb), torch.LongTensor(target_seq_noun), \
               a, b, c, d, labels
        pnr_value = []
        for name in d:
            assert name in self.pnr_mapping
            ...
            pnr_value.append(frames)
        pnr_data = torch.stack(pnr_value, dim=0)  # (num_inputs, 3, 16, 225, 225)
        return {'orig': orig_new, 'pnr': pnr_data}
