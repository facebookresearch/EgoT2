    ...
    def __next__(self) -> dict:
        ...
        for i_try in range(self._MAX_CONSECUTIVE_FAILURES):
            ...
            for clip_start, clip_end, clip_index, aug_index, is_last_clip in clips:
                # print(f'video name {video.name} start_sec {clip_start}, end_sec {clip_end}, duration {video._duration}')
                clip = video.get_clip(clip_start, clip_end)
                video_is_null = clip is None or clip["video"] is None
                if video_is_null:
                    print(f'video name {video.name} start_sec {clip_start}, end_sec {clip_end}, duration {video._duration} video_is_null {video_is_null}')
                if video_is_null:
                    break
...
def clip_recognition_dataset(
    ...
    if g_pathmgr.isfile(data_path):
        ...
        for entry in annotations['clips']:
            ...
            avg_duration.append(entry['action_clip_end_sec'] - entry['action_clip_start_sec'])
    else:
...
def clip_forecasting_dataset(
    ...
    return dataset, untrimmed_clip_annotations


def clip_ar_auxdataset(
        data_path: str,
        clip_sampler: ClipSampler,
        num_input_clips: int,
        input_offset: int,
    ...
    if g_pathmgr.isfile(data_path):
        ...
        for clip_uid, video_clips in annotations.items():
            ...
            for i in range(len(video_clips)):
                if i < num_input_clips + input_offset - 1:
                    continue
                    # input_clips = []
                    # for j in range(num_input_clips + input_offset):
                    #     input_clips.append(copy.deepcopy(video_clips[i: i + 1]))
                    # ar_clips = copy.deepcopy(video_clips[i: i + 1])
                else:
                    input_clips = copy.deepcopy(video_clips[i - num_input_clips - input_offset + 1: i + 1])
                    ar_clips = copy.deepcopy(video_clips[i - input_offset: i + 1])
                untrimmed_clip_annotations.append(
                    (
                        video_path,
                        {
