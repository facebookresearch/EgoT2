...
from dataset.ttm.data_loader_2task import ImagerSeqLoader2Task as TTMSeqDataset2Task
from dataset.asd.dataLoader import train_seqloader_2task as ASDSeqTrainDataset2Task
from dataset.asd.dataLoader import val_seqloader_2task as ASDSeqValDataset2Task
from dataset.ttm.sampler import SequenceBatchSampler
from models.multitask.task_prompt_model import TaskTranslationPromptTransformer
...
from utils.ttm.utils import collate_fn_2task_prompt
...
class Unified3TaskTranslation(LightningModule):
    def __init__(self, args):
        ...
        self.model = TaskTranslationPromptTransformer(args, self.vocab)
        self.criterion = nn.CrossEntropyLoss()
        self.asd_acc = Accuracy()

    def training_step(self, batch, batch_idx):
        video_lam, target_lam = batch['lam']
        video_ttm, video_ttm_aux, audio_ttm, audio_ttm_aux, target_ttm = batch['ttm']
        audio_asd, video_asd, video_asd_aux, target_asd = batch['asd']

        if len(audio_asd.shape) == 1:
            print('skipping this batch...')
            return

        logits_lam = self.model(video_lam, None, None, None, target_lam[:, :-1], 'lam')  # (bs, vocab_size-7, seq_len-2)
        loss_lam = self.criterion(logits_lam, target_lam[:, 1:])  # (bs, 2)
        try:
            logits_ttm = self.model(video_ttm, video_ttm_aux, audio_ttm, audio_ttm_aux, target_ttm[:, :-1], 'ttm')
        except ValueError as ve:
            print('!' * 100)
            print('Value Error', video_ttm.shape, video_ttm_aux.shape)
            return
        loss_ttm = self.criterion(logits_ttm, target_ttm[:, 1:])
        target_asd = target_asd[0].reshape(-1, 3)
        logits_asd = self.model(video_asd_aux[0], video_asd[0], audio_asd[0], audio_asd[0], target_asd[:, :-1], 'asd')
    ...
    def validation_step(self, batch, batch_idx):
        ...
        video_ttm, video_ttm_aux, audio_ttm, audio_ttm_aux, target_ttm, targetseq_ttm = batch_ttm[0], batch_ttm[1], \
                                                                                        batch_ttm[2], batch_ttm[3], \
                                                                                        batch_ttm[4][0], batch_ttm[4][1]
        audio_asd, video_asd, video_asd_aux, target_asd, targetseq_asd = batch_asd
        if len(audio_asd.shape) == 1:
            print('skipping this batch...')
            return

        logits_lam = self.model(video_lam, None, None, None, targetseq_lam[:, :-1], 'lam')
        loss_lam = self.criterion(logits_lam, targetseq_lam[:, 1:])
        output_lam = self.model.predict(video_lam, None, None, None, 'lam')

        if batch_idx < self.loader1_stop_idx:
            self.lam_postprocess.update(output_lam, target_lam)

        logits_ttm = self.model(video_ttm, video_ttm_aux, audio_ttm, audio_ttm_aux, targetseq_ttm[:, :-1], 'ttm')
        loss_ttm = self.criterion(logits_ttm, targetseq_ttm[:, 1:])
        output_ttm = self.model.predict(video_ttm, video_ttm_aux, audio_ttm, audio_ttm_aux, 'ttm')
        ...
        logits_asd = self.model(video_asd_aux[0], video_asd[0], audio_asd[0], audio_asd[0], targetseq_asd[:, :-1], 'asd')
        loss_asd = self.criterion(logits_asd, targetseq_asd[:, 1:])
        output_asd = self.model.predict(video_asd_aux[0], video_asd[0], audio_asd[0], audio_asd[0], 'asd')
    ...
    def train_dataloader(self):
        ...
        ttm_dataset = TTMSeqDataset2Task(self.vocab, args.ttm_img_path, args.ttm_wave_path, args.ttm_train_file, args.ttm_json_path,
        ...
        asd_dataset = ASDSeqTrainDataset2Task(vocab=self.vocab,
                                    trialFileName=os.path.join(self.file_path, "ego4d/csv/active_speaker_train.csv"),
    ...
    def val_dataloader(self):
        ...
        ttm_dataset = TTMSeqDataset2Task(self.vocab, args.ttm_img_path, args.ttm_wave_path, args.ttm_val_file, args.ttm_json_path,
        ...
        asd_dataset = ASDSeqValDataset2Task(vocab=self.vocab,
                                    trialFileName=os.path.join(self.file_path, "ego4d/csv/active_speaker_val.csv"),
                                    audioPath=os.path.join(self.file_path, "wave"),
                                    visualPath=os.path.join(self.file_path, "video_imgs"),
                                    batchSize=self.args.asd_batch_size)
